...
//
// It owns waitingForPoolMaintenance, held for the duration of the pass and
// released on every return path: the flag makes allocationNeeded() and
// releaseNeeded() false, and those gate the only two sites that enqueue this,
// so a pass returning with it set would strand the node for good.
