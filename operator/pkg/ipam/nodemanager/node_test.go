...
// MaintainIPPool: it owns waitingForPoolMaintenance for the duration of a pass
// and releases it on every return path, including the early ones. The flag
// gates allocationNeeded() and releaseNeeded(), which in turn gate the only two
// places that ever enqueue the pool maintainer, so a pass that returns with the
// flag still set strands the node for good.
