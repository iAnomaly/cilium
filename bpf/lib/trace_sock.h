 ...
 *
 * When aggregation is enabled (>=1), rate limiting aligns to report interval.
 */
			...
			bool is_connect)
{
	...
	struct ratelimit_settings settings = {
		.topup_interval_ns = CONFIG(ct_report).interval * NSEC_PER_SEC,
	};
	 ...
	 * Uses report interval as the time bucket for aggregation to
	 * align with monitor aggregation timing.
	 */
	if (CONFIG(monitor_aggregation) != TRACE_SOCK_AGGREGATE_NONE) {
		/* One token per report interval with no burst to align with
