						 ...
						 __u8 report_mask)
{
	 ...
	 */
	if (last_report + bpf_sec_to_mono(CONFIG(ct_report).interval) < now ||
 ...
 *
 * If report interval has elapsed since the last update, updates the
		...
		case ACTION_CLOSE:
			...
			*monitor = TRACE_PAYLOAD_LEN;
			...
			__ct_update_timeout(entry,
					    ...
					    dir, seen_flags, CONFIG(ct_report).flags);
			break;
		default:
			break;
		}
